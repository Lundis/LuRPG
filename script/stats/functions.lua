...
function calculate_attributes(playerId)
	...
	local str_1 = str_mult * (kills / 1000 + mined / 100 + built / 1000)
	local agi_1 = agi_mult * (walked / 100000)
	local dex_1 = dex_mult * (crafted / 100 + built / 1000)
	local int_1 = int_mult * (scans + researches + ghostBuild / 1000 + ghostMined / 1000)
	...
	local str = math.floor(math.log(1 + str_1, str_base))
	local agi = math.floor(math.log(1 + agi_1, agi_base))
	local dex = math.floor(math.log(1 + dex_1, dex_base))
	local int = math.floor(math.log(1 + int_1, int_base))
	...
	game.players[playerId].character_health_bonus = math.floor(       str +                        0.1*int)
	...
	game.players[playerId].character_inventory_slots_bonus = math.min(math.floor((str)/5), 140)
